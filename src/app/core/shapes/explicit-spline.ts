...
export class ExplicitSpline extends AbstractSpline {
	...
	hideLines () {
		...
		this.lines.forEach( segment => segment.visible = false );

	}

	showLines () {

		this.lines.forEach( segment => segment.visible = true );
	...
	update (): void {
		...
		for ( let i = 0; i < this.lines.length; i++ ) {
			...
			this.updateLineBufferAttributes( i );
	...
	addControlPoint ( controlPoint: RoadControlPoint ) {
		...
		const index = controlPoint.tagindex || this.segTypes.length;
		...
		const hdg = this.calculateHdg( index, controlPoint.position );
		...
		if ( index > 0 ) {
			...
			this.updateLineBufferAttributes( index - 1 );
	...
	removeControlPoint ( cp: RoadControlPoint ) {
		...
		const index = cp.tagindex || this.controlPoints.indexOf( cp );
	...
	insertPoint ( point: RoadControlPoint ): void {

		const index = this.findIndex( point );

		point.tagindex = index;
	...
	addControlPointAt ( position: Vector3 ) {
		...
		const hdg = this.calculateHdg( index, position );
	...
	updateRoadSegments () {
		...
		for ( const segment of this.splineSegments ) {
	...
	private addLine ( index: number, abstractRoadGeometry?: TvAbstractRoadGeometry ) {
	...
	private updateLineBufferAttributes ( index: number ) {

		const line = this.lines[ index ];

		const bufferAttribute = line.geometry.attributes.position;

		const dir1 = new Vector2( Math.cos( this.hdgs[ index ][ 0 ] ), Math.sin( this.hdgs[ index ][ 0 ] ) );
		const dir2 = new Vector2( Math.cos( this.hdgs[ index + 1 ][ 0 ] ), Math.sin( this.hdgs[ index + 1 ][ 0 ] ) );

		const p1 = this.controlPointPositions[ index ];
		const p2 = this.controlPointPositions[ index + 1 ];

		const distance = p1.distanceTo( p2 );

		if ( this.segTypes[ index ] == TvGeometryType.LINE ) {

			bufferAttribute.setXYZ( 0, p1.x, p1.y, p1.z );
			bufferAttribute.setXYZ( 1, p2.x, p2.y, p2.z );

			for ( let ii = 2; ii < CURVE_TESSEL; ii++ ) {
				bufferAttribute.setXYZ( ii, p2.x, p2.y, p2.z );
			}

			bufferAttribute.needsUpdate = true;

		} else if ( this.segTypes[ index ] == TvGeometryType.ARC ) {
			...
			for ( let ii = 0; ii < CURVE_TESSEL; ii++ ) {
				...
				const d = distance * Math.sin( theta / 2 ) / Math.sin( alpha / 2 );
				...
				bufferAttribute.setXYZ( ii, p1.x + advx.x * Math.cos( gamma ) * d + advy.x * Math.sin( gamma ) * d, p1.y + advx.y * Math.cos( gamma ) * d + advy.y * Math.sin( gamma ) * d, CURVE_Y );
			}

			bufferAttribute.needsUpdate = true;

		} else if ( this.segTypes[ index ] == TvGeometryType.SPIRAL ) {
			...
			for ( let ii = 0; ii < CURVE_TESSEL; ii++ ) {
				...
				bufferAttribute.setXYZ( ii, spiralarc[ ii ][ 0 ] / 100, spiralarc[ ii ][ 1 ] / 100, 0 );
			}

			bufferAttribute.needsUpdate = true;

		} else if ( this.segTypes[ index ] == TvGeometryType.PARAMPOLY3 ) {

			const t1 = new Vector3( dir1.x, dir1.y, CURVE_Y ).multiplyScalar( PARACUBICFACTOR * this.hdgs[ index ][ 1 ] );
			const t2 = new Vector3( dir2.x, dir2.y, CURVE_Y ).multiplyScalar( PARACUBICFACTOR * this.hdgs[ index + 1 ][ 2 ] );
			...
			for ( let ii = 0; ii < CURVE_TESSEL; ii++ ) {
				...
				bufferAttribute.setXYZ( ii, posfoo.x, posfoo.y, posfoo.z );
			}
	...
	private exportFromSpline ( segTypes: number[], hdgs: number[][], points: Vector3[], duringImport = false ): TvAbstractRoadGeometry[] {
		...
		for ( let i = 0; i < this.lines.length; i++ ) {
			...
			const currentPoint = this.controlPoints[ i ] as RoadControlPoint;
			const nextPoint = this.controlPoints[ i + 1 ] as RoadControlPoint;
