...
// Module-level reusable vectors and matrices to avoid memory allocation
...
export interface SelectionBoxInstances {
	[ uuid: string ]: number[];
}
...
export class SelectionBox
{
	...
	public instances: SelectionBoxInstances;
	public deep: number;
	...
	public select ( startPoint?: Vector3, endPoint?: Vector3 ): Object3D[] {
	...
	public searchChildInFrustum ( frustum: Frustum, object: Object3D ): void {
		const isMesh = ( object as Mesh ).isMesh;
		const isLine = ( object as Line ).isLine;
		...
		if ( isMesh || isLine || isPoints ) {
			...
			if ( instancedMesh.isInstancedMesh ) {
				...
				for ( let instanceId = 0; instanceId < instancedMesh.count; instanceId++ ) {
					instancedMesh.getMatrixAt( instanceId, _matrix );
