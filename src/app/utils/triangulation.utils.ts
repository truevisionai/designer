...
export function weilerAtherton ( vertices: number[], edges: number[], clipPolygon: number[][] ): number[] {
	...
	function findVertexIndex ( vertices, point ): number {
		for ( let i = 0; i < vertices.length; i += 2 ) {
	...
	function clipTriangle ( triangle, clipPolygon ): any {
		...
		function inside ( p, cp1, cp2 ): boolean {
...
export function fixWindingOrder ( vertices, triangles ): any[] {
